...
function buildAppScriptPath(appName) {
  return path.resolve(__dirname, `../script/${appName}.js`);
}

function flushAppScript(appName, script) {
  const scriptPath = buildAppScriptPath(appName);
  fs.ensureFileSync(scriptPath);
  fs.outputFileSync(scriptPath, script);
  return scriptPath
}
...
router.get('/:appName/deployments/:deploymentName/region/:regionName',
  middleware.checkToken, (req, res, next) => {
    var uid = req.users.id;
    var appName = _.trim(req.params.appName);
    var deploymentName = _.trim(req.params.deploymentName);
    var regionName = _.trim(req.params.regionName);
    var deployments = new Deployments();
    accountManager.collaboratorCan(uid, appName)
      .then((col) => {
        return deployments.findDeloymentByName(deploymentName, col.appid)
      ...
      .then((deploymentInfo) => {
        ...
        res.send({deployment: deployments.listDeloymentAndRegion(deploymentInfo, regionName)});
        return true;
      })
      .then()
      .catch((e) => {
...
router.delete('/:appName/deployments/:deploymentName/region/:regionName',
  middleware.checkToken, (req, res, next) => {
    var uid = req.users.id;
    var appName = _.trim(req.params.appName);
    var deploymentName = _.trim(req.params.deploymentName);
    ...
    accountManager.ownerCan(uid, appName)
      ...
      .then((deploymentInfo) => {
        if (_.isEmpty(regionName)) {
          throw new AppError.AppError("does not find the regionName");
        }
        return deployments.deleteDeploymentRegion(deploymentInfo.id, regionName);
      })
      .then((rs) => {
        res.send("ok");
      })
...
router.delete('/:appName/deployments/:deploymentName/free/:regionName',
  middleware.checkToken, (req, res, next) => {
    var appName = _.trim(req.params.appName);
    var deploymentName = _.trim(req.params.deploymentName);
    var regionName = _.trim(req.params.regionName || "");
    ...
    accountManager.ownerCan(uid, appName)
      ...
      .then((deploymentInfo) => {
        ...
        return packageManager.freePackage(deploymentInfo.id, regionName);
      })
      .then((data) => {
        res.send({msg: data});
      })
  ...
  middleware.checkToken, (req, res, next) => {
    ...
    accountManager.collaboratorCan(uid, appName)
      .then((col) => {
        ...
        return deployments.findDeloymentByName(deploymentName, col.appid)
          .then((deploymentInfo) => {
            const deploymentKey = deploymentInfo.deployment_key;
            if (_.isEmpty(deploymentInfo)) {
            ...
            return packageManager.parseReqFile(req)
              .then((data) => {
                ...
                return packageManager.releasePackage(deploymentInfo.appid, deploymentInfo.id, data.packageInfo, data.package.path, uid, data.regionName)
                  .finally(() => {
...
router.patch('/:appName/deployments/:deploymentName/release',
  middleware.checkToken, (req, res, next) => {
    ...
    var scriptBase64 = _.get(req, 'body.script');
    accountManager.collaboratorCan(uid, appName)
      .then((col) => {
        return deployments.findDeloymentByName(deploymentName, col.appid)
          .then((deploymentInfo) => {
...
var rollbackCb = function (req, res, next) {
  ...
  var regionName = _.trim(_.get(req, 'params.region'));
  var deployments = new Deployments();
  var packageManager = new PackageManager();
  accountManager.collaboratorCan(uid, appName)
    .then((col) => {
    ...
    .then((dep) => {
      return packageManager.rollbackPackage(dep.last_deployment_version_id, targetLabel, uid, regionName)
        .then((packageInfo) => {
          if (packageInfo) {
            Promise.delay(1000)
              .then(() => {
...
router.post('/:appName/deployments/:deploymentName/rollback/:label/:region',
  middleware.checkToken, rollbackCb);
    ...
    if (_.isEmpty(newAppName)) {
      ...
      return accountManager.ownerCan(uid, appName)
        ...
        .then(() => {
          res.send("");
          // 根据新应用名修改规则脚本
          const oldScriptPath = buildAppScriptPath(appName)
          if (fs.existsSync(oldScriptPath)) {
            fs.renameSync(oldScriptPath, buildAppScriptPath(newAppName))
          }
        })
        .catch((e) => {
          if (e instanceof AppError.AppError) {
...
router.post('/', middleware.checkToken, (req, res, next) => {
  ...
  const script = Buffer.from(s, 'base64').toString('utf8');
