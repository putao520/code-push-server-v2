...
proto.createPackage = function (deploymentId, appVersion, region, packageHash, manifestHash, blobHash, params) {
...
proto.releasePackage = function (appId, deploymentId, packageInfo, filePath, releaseUid, region) {
  var self = this;
  ...
  return Promise.all([
    ...
    .then((blobHash) => {
      ...
      return dataCenterManager.storePackage(directoryPath)
        .then((dataCenter) => {
          var packageHash = dataCenter.packageHash + region;
          ...
          return models.DeploymentsVersions.findOne({where: {deployment_id: deploymentId, app_version: appVersion}})
            .then((deploymentsVersions) => {
              ...
              // 如果 region 作为hash的一部分，那么就不需要判断 region
    ...
    .spread((packageHash, manifestHash, blobHash) => {
      ...
      return self.createPackage(deploymentId, appVersion, region, packageHash, manifestHash, blobHash, params);
    })
...
proto.rollbackPackage = function (deploymentVersionId, targetLabel, rollbackUid, region) {
  var self = this;
  return models.DeploymentsVersions.findById(deploymentVersionId)
    .then((deploymentsVersions) => {
      ...
      // 包含区域信息
      const r = (region && !_.isEmpty(region)) ?
        models.Packages.findOne({where: {deployment_version_id: deploymentVersionId, region: region}}) :
        models.Packages.findById(deploymentsVersions.current_package_id);

      return r.then((currentPackageInfo) => {
        if (targetLabel) {
          const query = {deployment_version_id: deploymentVersionId, label: targetLabel}
          if (region) {
            query.region = region
          }
          return models.Packages.findAll({where: query, limit: 1})
...
proto.freePackage = function (deploymentId, region) {
  var self = this;
  return models.Packages.destroy({where: {deployment_id: deploymentId, region: region}})
}
